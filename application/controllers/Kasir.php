...
class Kasir extends CI_Controller {
	...
	public function barang() {
		$data['kategori'] = $this->kasir_model->getKategory();
		$data['satuan'] = $this->kasir_model->getSatuan();
		$data['supplier'] = $this->kasir_model->getSupplier();
		$this->load->view('header', $data);
	...
	public function json_produk() {
		if ($this->input->is_ajax_request()) {
			$this->kasir_model->getProduk();
		} else {
			redirect('kasir/barang/', 'refresh');
		}
	}
	...
	public function stok() {
		...
		if ($kat != "wow" && $sort) {
			if ($sort == "empty") {
				$data['stok'] = $this->kasir_model->getStokEmpty($kat);
			} elseif ($sort == "more") {
				$data['stok'] = $this->kasir_model->getStokMore($kat);
			} else {
				$data['stok'] = $this->kasir_model->getStokSort($kat);
			}
		} elseif ($kat == "wow" && $sort == "all") {
			$data['stok'] = $this->kasir_model->getStokAll();
		} elseif ($kat == "wow" && $sort == "more") {
			$data['stok'] = $this->kasir_model->getStok();
		} elseif ($kat == "wow" && $sort == "empty") {
			$data['stok'] = $this->kasir_model->getStokAllEmpty();
		} else {
			$data['stok'] = $this->kasir_model->getStok();
		}
		$data['sort'] = $sort;
		$data['kat'] = $kat;
		$this->load->view('header', $data);
		$this->load->view('kasir/stok');

	}

	public function stok_min() {
		$data['tgl'] = date('d M Y');
		$data['stok'] = $this->kasir_model->getStokMin();
		$this->load->view('header', $data);
		$this->load->view('kasir/stok_min');
	}
...
public function add_list_penjualan() {
		...
		$harga_modal = $this->input->post('hrg_beli');
		$stok = $this->input->post('stok');
		$diskonrp = "0";
		$diskonpersen = "0";
		$produk = $this->kasir_model->getbarang($kd_barang);
			
		$subtotal = (int) $hrg_jual * (int) $jumlah;
// ---------------------Jika kode barang d list sudah ada update jumlah dan subtotal----------
		$cek_sudah_ada = $this->kasir_model->cek_sudah_ada($kd_barang, $nofaktur);
		...
		if ($produk->num_rows() > 0) {
			...
			if ($cek_sudah_ada->num_rows() > 0) {
				...
				$subtot_sekarang = $hrg_jual * $jum_beli_sekarang;
				if ($jum_beli_sekarang > $stok_sekarang) {
					echo $this->session->set_flashdata('error', 'Stok barang tidak cukup');
					header("Location: " . $uri, TRUE, $http_response_code);
				} else {
					$this->db->query("UPDATE tabel_rinci_penjualan SET jumlah='$jum_beli_sekarang', sub_total_jual='$subtot_sekarang', diskonrp='$diskonrp', diskonpersen='$diskonpersen' WHERE kd_barang='$kd_barang' AND no_faktur_penjualan='$nofaktur'");
					header("Location: " . $uri, TRUE, $http_response_code);
				}
			} else {
				if ($stok_sekarang < $jumlah) {
					echo $this->session->set_flashdata('error', 'Kasir cek : Stok Kosong! ');
					header("Location: " . $uri, TRUE, $http_response_code);
				} else {
					if ($subtotal < 0) {
			...
			foreach ($list_produk as $key) {
				...
				if ($stok_sekarang > $jumlah_item) {
					...
					foreach ($stok_porsi as $key) {
						...
						$stok_baru = (int) $stok_barang - (int) $jumlah_item;
						$this->db->query("UPDATE tabel_stok_toko SET stok='$stok_baru' WHERE kd_barang='$kd_barang'");
						$this->db->query("INSERT INTO tabel_kartu_stok (kode_toko,kode_barang,waktu,jam,sebelumnya,keluar,masuk,saldo,keterangan,user,publish) VALUES ('$kd_toko','$kd_barang','$tgl','$waktu','$stok_barang','$jumlah_item','0','$stok_baru','$ket_ks','$id_user','0')");
					}
				}
			};
			$update = array(
